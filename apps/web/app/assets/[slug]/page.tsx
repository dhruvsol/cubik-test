    ...
    return (
        <>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {response?.results?.map((data, index) => (
                    <div key={index} className="bg-white rounded-lg shadow-lg p-4">
                        <img src={data?.content?.files![0].uri} alt="Image" className="w-full mb-4" />
            ...
            <div className="fixed bottom-0 left-0 right-0 flex justify-center items-center bg-gray-200 p-4 gap-2">
                {Number(page) !== 1 && <a href={`/assets/${Number(page) - 1}`}><Button className="border border-black" variant="outline">Previous (Page {Number(page) - 1})</Button></a>}
                <a href={`/assets/${Number(page) + 1}`}><Button>Next (Page {Number(page) + 1})</Button></a>
            </div>
        </>
    )
}
